/*
 * Assignment: Create an Airport counter class to maintain all outgoing
 * flights from given airport. Unit tests should take
 *  no longer then 2 sec in total.
 * */
...
//
//	MyPair() :  first(static_cast<T>((count++)%CAST_UINT32(T::Last_))),second(U()){}
//
//};


//template<typename T,typename U>
//uint32_t MyPair<T,U>::count = 0;
...
public:
  ...
  //Templated constructor , updating outgoing flights based on iterators range
  template <typename TIterator>
  AirportCounter(TIterator begin, TIterator end) : AirportCounter(){
	  for (auto& it=begin;it !=end;it++){
		  Airports[CAST_UINT32(*it)]++;
	  }
  }

  //Get number of outgoing flights for one airport
  size_t Get(TAirport airport) const{
	  return Airports[CAST_UINT32(airport)];
  }

  //Insert a new outgoing flight for a given airport
  void Insert(TAirport airport){
	  Airports.at(CAST_UINT32(airport))++;
  }

  //Remove one flight for a given airport
  void EraseOne(TAirport airport){
	  Airports.at(CAST_UINT32(airport))--;
  }

  //Remove all outgoing flights for one airport
  void EraseAll(TAirport airport){
	  Airports.at(CAST_UINT32(airport)) = 0;
  }

  using Item = pair<TAirport, size_t>;
  using Items = array<size_t,CAST_UINT32(TAirport::Last_)>;

  //Return a container with pairs of TAirport, size_t
  const array<Item,CAST_UINT32(TAirport::Last_)>  GetItems() const{
	  array<Item,CAST_UINT32(TAirport::Last_)> return_db;


	  for (auto i= 0;i<Airports.size();i++){
		  return_db[i].first = static_cast<TAirport>(i);
		  return_db[i].second = Airports.at(i);
	  }
	  return return_db;
  }
...
void TestMostPopularAirport() {
  ...
  const int days_to_explore = 2;//365 * 500;
